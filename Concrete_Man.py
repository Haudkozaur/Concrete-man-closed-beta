...
def game():
    global score_board_file, rebar_state, playerY, playerX, playerX_change, playerY_change, kierownikchujX, kierownikchujY, kierownikchujY_change, kierownikchujX_change, bhpX_change, bhpY_change, bhpX, bhpY, rebarX, rebarY, score, hp, game_state, time, current_time, rebar_fallX, rebar_fallY, run, game_over, hitbox_corrector
    game_state = "will see"
    ...
    zupa = True
    click = False
    score_board_file = "highscore.txt"
    path1 = "C:\\Users\\Julian Haudek\\PycharmProjects\\pythonProject3"

    def load_data():
        global score_board_file, path1
        path1 = "C:\\Users\\Julian Haudek\\PycharmProjects\\pythonProject3"

        with open(path.join(path1, score_board_file), 'w') as f:
            try:
                lowtime = int(f.read())
            except:
                lowtime = 0
    ...
    def main_menu():
        global run, pupa, zupa, click, hitbox_corrector
        zupa = True
        while zupa:
    ...
    def press_any_key_lvl1():
        global pupa, zupa, run, hitbox_corrector
        while pupa:
    ...
    def level1():
        global path1, rebar_state, playerY, playerX, playerX_change, playerY_change, kierownikchujX, kierownikchujY, \
            kierownikchujY_change, kierownikchujX_change, bhpX_change, bhpY_change, bhpX, bhpY, rebarX, rebarY, score, hp, game_state, time, current_time, rebar_fallX, rebar_fallY, run, game_over, hitbox_corrector
        while run:
            # RGB
            # screen.fill((120, 120, 120))
            ...
            for event in pygame.event.get():
                ...
                if rebar_state == "picked":
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_LEFT or event.key == pygame.K_a:
                            playerX_change = -basic_player_velocity / 2
                        if event.key == pygame.K_RIGHT or event.key == pygame.K_d:
                            playerX_change = basic_player_velocity / 2
                        if event.key == pygame.K_UP or event.key == pygame.K_w:
                ...
                for i in range(num_of_formworks):
                    ...
                    if rebar_state == "delivered":
                        hitbox_formwork_player = hitbox(playerX, playerY, formworkX[i] + hitbox_corrector,
                                                        formworkY[i] + hitbox_corrector)
                        hitbox_formwork_rebar = hitbox(rebarX, rebarY, formworkX[i] + hitbox_corrector,
                                                       formworkY[i] + hitbox_corrector)
                    if rebar_state == "forsaken":
                        hitbox_formwork_player = hitbox(playerX, playerY, formworkX[i] + hitbox_corrector,
                                                        formworkY[i] + hitbox_corrector)
                        hitbox_formwork_rebar = hitbox(rebar_fallX, rebar_fallY, formworkX[i] + hitbox_corrector,
                                                       formworkY[i] + hitbox_corrector)
